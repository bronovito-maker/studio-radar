...
export default async function SettingsPage({ searchParams }: SettingsPageProps) {
  ...
  const { data: settings } = await supabase
    .from("settings")
    .select("booking_url, default_score_threshold, cron_enabled, cron_category, cron_location, cron_region, cron_schedule, email_enabled, email_sender_name, email_sender_email, email_reply_to, email_daily_limit, email_follow_up_enabled, email_follow_up_delays")
    .eq("id", 1)
    .single();
