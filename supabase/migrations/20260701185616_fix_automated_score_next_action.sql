-- Scoring V2 exposes nextAction at scoringV2.nextAction. Keep compatibility
-- with the earlier nested recommendation shape while preferring the current one.
...
declare
  ...
  v_score_id uuid;
  v_threshold integer;
  v_next_action text := coalesce(
    p_input_snapshot #>> '{scoringV2,nextAction}',
    p_input_snapshot #>> '{scoringV2,recommendation,nextAction}'
  );
...
begin
  ...
  insert into public.lead_scores (
    lead_id, score, grade, recommended_service_id, reasoning,
    positive_signals, negative_signals, deterministic_score, ai_score,
    confidence, provider, model, prompt_version, input_snapshot, created_by
  ) values (
    p_lead_id, p_score, p_grade, v_service_id, trim(p_reasoning),
    coalesce(p_positive_signals, '{}'), coalesce(p_negative_signals, '{}'),
    p_score, null, p_confidence, 'deterministic', null, trim(p_version),
    coalesce(p_input_snapshot, '{}'::jsonb), v_actor_id
  )
  returning id into v_score_id;

  update public.leads
  set recommended_service_id = v_service_id,
      status = case
        when status = 'new'
