-- Persist deterministic scoring history and qualification atomically.
...
begin
  ...
  set
    ...
    status = case
      when status = 'new' and p_score >= coalesce(v_threshold, 50) then 'qualified'::public.lead_status
      else status
    end
  where id = p_lead_id
